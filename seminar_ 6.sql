...
-- процедура, с помощью которой можно переместить 
-- любого (одного) пользователя из таблицы users в таблицу users_old.
DROP PROCEDURE IF EXISTS transfer_user;
DELIMITER //
CREATE PROCEDURE transfer_user(value BIGINT, OUT  out_result varchar(100))
BEGIN
	
	DECLARE `_rollback` TINYINT UNSIGNED DEFAULT 0;
	...
	START TRANSACTION;
		SELECT count(u.id) FROM users u  WHERE u.id=value INTO @user_en;
		IF @user_en=1  THEN
			INSERT INTO users_old(firstname, lastname, email) SELECT firstname, lastname, email FROM users WHERE users.id = value;
			DELETE FROM users WHERE users.id = value;
		ELSE
			SET `_rollback` = 2;
		END IF;
	...
	IF `_rollback`=1 THEN
		SET out_result = CONCAT('УПС. Ошибка: ', code, ' Текст ошибки: ', error_string);
		ROLLBACK;
	ELSEIF `_rollback`=2 THEN
		SET out_result = 'Такого Id пользователя нет';
		ROLLBACK;
	ELSE
		SET out_result = 'OK';
		COMMIT;
...
-- вызов процедуры
-- transfer_user(id пользователя, @результат выполнения);
CALL transfer_user(3, @out_result); 
SELECT @out_result;
...
BEGIN
	DECLARE return_value varchar(100); 	-- возвращаемая переменная
	DECLARE time_value INT;		   		-- переменная текущего времени	
	
	SET time_value = (
		SELECT CURTIME() + 0 
		);
	
	IF time_value BETWEEN 60001 AND 120000 THEN
		SET return_value = 'Доброе утро';
	ELSEIF time_value BETWEEN 120001 AND 180000 THEN
		SET return_value = 'Добрый день';
	ELSEIF time_value >= 180001 OR time_value = 0 THEN
		SET return_value = 'Добрый вечер';
	ELSE
		SET return_value = 'Доброй ночи';
	END IF;
...
FOR EACH ROW
BEGIN
	INSERT INTO logs (name_table, write_id, date_time) VALUES 
	('users', NEW.id, NOW());
END//
DELIMITER ;
...
FOR EACH ROW
BEGIN
	INSERT INTO logs (name_table, date_time) VALUES 
	('messages', NOW());
END//
DELIMITER ;
...
('Test', 'Nienow', 'test@example.org');

INSERT INTO communities (name) 
VALUES ('test_communies');

INSERT INTO messages  (from_user_id, to_user_id, body, created_at) VALUES
(1, 5, 'it et. Nam accusantium aut qui quae nesciunt non.',  DATE_ADD(NOW(), INTERVAL 1 MINUTE));

SELECT name_table, write_id, date_time FROM logs;
