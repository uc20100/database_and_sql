...
CREATE OR REPLACE VIEW v_user_max_age_20 AS  
SELECT u.firstname, u.lastname, p.hometown, p.gender FROM users u  
JOIN profiles p ON u.id = p.user_id 
WHERE TIMESTAMPDIFF(YEAR, p.birthday , curdate()) <= 20; 

SELECT * FROM v_user_max_age_20;
...
SELECT 
	DENSE_RANK() OVER(ORDER BY COUNT(m.from_user_id) DESC) AS rank_user,
...
-- 	m.body,
	m.created_at, 
	CONCAT(DATEDIFF(m.created_at, LEAD(m.created_at) OVER (ORDER BY m.created_at DESC)), ' дней ', 
	       TIMEDIFF(m.created_at, LEAD(m.created_at) OVER (ORDER BY m.created_at DESC)), ' часов/минут/секунд ') AS 'Разница дат'
FROM messages m;
