...
-- Задаем id пользователя которого хотим отследить
SET @to_user := 1;
...
SELECT
	...
	COUNT(u.id) AS n_mes
FROM messages m
-- выбираем сообщения с отслеживаемым пользователем
JOIN users u ON u.id = m.from_user_id  
AND m.to_user_id = @to_user   
-- выбираем только друзей пользователя
JOIN friend_requests fr ON (u.id = fr.initiator_user_id OR u.id = fr.target_user_id)
AND fr.status = 'approved' AND (fr.initiator_user_id = @to_user OR fr.target_user_id = @to_user) 
-- группируем по полю m.from_user_id и выводим по убыванию
GROUP BY m.from_user_id
ORDER BY n_mes DESC LIMIT 1;
